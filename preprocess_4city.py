...
def preprocess_tky_ca(args:any,cfg: Cfg, path: bytes) -> pd.DataFrame:
    if args.dataset_name in ['Moscow','SaoPaulo','Shanghai','TKY']:
        raw_file = f'dataset_TSMC2014_{args.dataset_name}.txt'
    else:
        raw_file = 'dataset_gowalla_ca_ne.csv'

    FileReader.root_path = path
    data = FileReader.read_dataset(raw_file, args.dataset_name)
...
def preprocess(args, cfg: Cfg):
    ...
    if not osp.exists(sample_file):
        ...
        elif dataset_name in ['Moscow','SaoPaulo','Shanghai','TKY']:
            preprocessed_data = preprocess_tky_ca(args, cfg, data_path)
        else:
...
if __name__ == '__main__':
    ...
    # Parse arguments
    conf_file = "best_conf/agentmove.yml"
